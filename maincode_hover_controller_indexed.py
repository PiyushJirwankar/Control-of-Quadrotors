...
for i in range(0,N):  #position control loop
    ...
    for j in range(10):
        ...
        if (10*i+j == 10*N-1):             #If statement to prevent the index error for the last index in propagation step
            ...
            #omega_b[:, 1999] = RK4(dynamics, omega_b[:, 1999], time_att[1000], time_att[1999] ,0.0001/100 , 3)[0][:, 1999] 
            omega_b[:, 10*i+j+1] = RK4(dynamics, omega_b[:, 10*i+j], time_att[10*i+j], time_att[10*i+j+1] ,0.0001/100 , 3)[0][:,-1]# 10*i+j]
            # print(np.shape(RK4(dynamics, omega_b[:, 10*i+j], time_att[10*i+j], time_att[10*i+j+1] ,0.0001/100 , 3)[0]))
            #print("shape", np.shape(omega_b[:, 10*i+j+1]))
                #h=0.0001/100
            att[:, 10*i+j+1] = att[:, 10*i+j] + omega_b[:, 10*i+j]*0.0001
    R = rotm(att[0,10*i], att[1, 10*i], att[2, 10*i])
    vec = np.array([0, 0, control_inputs[0, 10*i+j]])
...
# print("len r_dot_dot",np.shape(r_dot_dot[0,:]))
# print("len time_pos", len(time_pos_plt))

plt.plot(time_att_plt, omega_b[0,:])
plt.plot(time_pos_plt, r_dot_dot[0,:])
