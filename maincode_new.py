...
r_dot_dot_T = np.zeros((3,N+2))           #acceleration to be traked at all times
r_dot_T     = np.zeros((3,N+2))           #velocity to be traked at all times
r_T         = np.zeros((3,N+2))           #position to be tracked at all times
for i in range(N):
...
r               = np.zeros((3,N+2))       #position at all times        
r[:,0]          = np.array([0, 0, 0])   #initialising r at t=0

r_dot           = np.zeros((3,N+2))       #velocity at all times
r_dot[:,0]      = np.array([0, 0, 0])   #initialising r_dot at t=0

r_dot_dot       = np.zeros((3,N+2))       #acceleration at all times
...
att_T = np.zeros((3, 10*N+20))             # [phi_T, theta_T, psi_T]    atttude trajectory to be  tracked
for i in range(10*N):
    att_T[0, i] = 0                     #phi_T = 0 at all times
    att_T[1, i] = 0                     #theta_T = 0 at all times
    att_T[2, i] = 0                     #psi_T = 0 at all times
    
att = np.zeros((3, 10*N+20))               #attitude [phi, theta, psi] at all times
...
control_inputs = np.zeros((4, 10*N+2))    #[u1, u2, u3, u4]




time_att = np.linspace(0, 5.001, 5001)
#time_att = time_att[0:5000]
time_pos = np.linspace(0, 5.01, 501)
...
for i in range(0,N):  #position control loop
    ...
    for j in range(10):
        ...
        def dynamics(t,y):
            p = control_inputs[0,i]/Ixx - omega_b[1,i]*omega_b[2,10*i+j]*(Izz-Iyy)
            q = control_inputs[1,i]/Iyy - omega_b[0,i]*omega_b[2,10*i+j]*(Ixx-Izz)
            r = control_inputs[2,i]/Izz
            return np.array([p, q, r])
        omega_b[:, 10*i+j+1] = RK4(dynamics, omega_b[:, 10*i+j], time_att[10*i+j], time_att[10*i+j+1] ,0.0001/100 , 3)[0][:, 10*i+j] 
            #h=0.0001/100
